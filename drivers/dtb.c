...
// Extract path or alias from stdout-path value (strip options like ":115200n8"
// or ",keep")
static void extract_path_token(const char *val, uint32_t len,
                               const char **out_start, uint32_t *out_len) {
...
static uint64_t dtb_search(void) {
    ...
    static const uint64_t locations[5] = {0x48000000, // 128MB from RAM start
    ...
    uint64_t start =
        ((uint64_t)_kernel_end + 0xfffff) & ~0xfffff; // Align to 1MB
...
struct dtb_header *dtb_get() { return (struct dtb_header *)dtb_ptr; }
...
int dtb_get_stdout_uart_base(uint64_t *out_base) {
    ...
    const char *stdout_token = NULL;
    uint32_t stdout_token_len = 0;
    ...
    while (1) {
        ...
        if (token == DTB_BEGIN_NODE) {
            ...
            if (depth < 16)
                current_path_stack[depth++] = name;

            // Determine if we are inside /chosen or /aliases
            int in_chosen = 0, in_aliases = 0;
            if (depth >= 1) {
                // Reconstruct minimal check: at depth 1, name should be ""
                // (root) At depth 2, name[1] is first-level node, so simple
                // compare
                if (depth == 2 && name) {
                    if (strncmp(name, "chosen", 6) == 0)
                        in_chosen = 1;
                    if (strncmp(name, "aliases", 7) == 0)
                        in_aliases = 1;
                }
            }
            ...
            while (1) {
                ...
                if (ptok == DTB_PROP) {
                    ...
                    uint32_t len = be32_to_cpu(*(uint32_t *)(fdt + p));
                    p += 4;
                    uint32_t nameoff = be32_to_cpu(*(uint32_t *)(fdt + p));
                    p += 4;
                    const char *pname = strings + nameoff;
                    const char *pdata = (const char *)(fdt + p);

                    if (in_chosen && (strncmp(pname, "stdout-path", 11) == 0 ||
                                      strncmp(pname, "stdout", 6) == 0)) {
                        extract_path_token(pdata, len, &stdout_token,
                                           &stdout_token_len);
                        ...
                        aliases[alias_count].path =
                            pdata; // e.g., "/soc/serial@..."
                        alias_count++;
                    }
                    ...
                    // nested node under /chosen or /aliases; handle in outer
                    // loop back up 4 to let outer loop process this token as a
                    // new node
                    p -= 4;
                    break;
                } else {
...
pass1_done:
    ...
    if (stdout_token[0] == '/') {
        // copy path
        uint32_t n = (stdout_token_len < sizeof(resolved_path_buf) - 1)
                         ? stdout_token_len
                         : (uint32_t)sizeof(resolved_path_buf) - 1;
        for (uint32_t i = 0; i < n; ++i)
            resolved_path_buf[i] = stdout_token[i];
        ...
        for (int i = 0; i < alias_count; ++i) {
            ...
            size_t j = 0;
            while (an[j] && an[j] != '\0' && stdout_token[j] &&
                   j < stdout_token_len && an[j] == stdout_token[j])
                j++;
            if (j == stdout_token_len && an[j] == '\0') {
                // copy alias path
                const char *ap = aliases[i].path;
                size_t k = 0;
                while (ap[k] && k < sizeof(resolved_path_buf) - 1) {
                    resolved_path_buf[k] = ap[k];
                    k++;
                }
    ...
    while (1) {
        ...
        if (token == DTB_BEGIN_NODE) {
            ...
            if (!(depth == 0 && name_len == 0)) { // root has empty name
                size_t cur_len = 0;
                while (current_path[cur_len] != '\0')
                    cur_len++;
                if (cur_len > 1)
                    current_path[cur_len++] = '/';
                for (size_t i = 0;
                     i < name_len && cur_len < sizeof(current_path) - 1; ++i)
                    current_path[cur_len++] = name[i];
                current_path[cur_len] = '\0';
            }
            depth++;

            int at_target =
                target_path && path_equals(current_path, target_path);
            while (1) {
                uint32_t ptok = be32_to_cpu(*(uint32_t *)(fdt + p));
                p += 4;
                if (ptok == DTB_PROP) {
                    ...
                    if (at_target && strncmp(pname, "reg", 3) == 0) {
                        ...
                        if (len >= 8) {
                            uint32_t hi =
                                be32_to_cpu(*(const uint32_t *)(pdata + 0));
                            uint32_t lo =
                                be32_to_cpu(*(const uint32_t *)(pdata + 4));
                    ...
                    while (current_path[cur_len] != '\0')
                        cur_len++;
                    while (cur_len > 1 && current_path[cur_len - 1] != '/')
                        cur_len--;
                    if (cur_len > 1)
