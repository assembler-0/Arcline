...
static int test_pmm_stress(void) {
    ...
    for (int i = 0; i < STRESS_PAGES; i++) {
        ...
        if (!pages[i]) {
            printk(" FAIL (alloc %d)\n", i);
            for (int j = 0; j < i; j++) pmm_free_page(pages[j]);
            return -1;
        }
    ...
    for (int i = 0; i < STRESS_PAGES; i++) {
        ...
        for (int j = 0; j < 4096; j++) {
            if (buf[j] != (uint8_t)(i & 0xFF)) {
                ...
                for (int k = 0; k < STRESS_PAGES; k++) pmm_free_page(pages[k]);
                return -1;
            }
        }
    }
...
static int test_vmm_basic(void) {
    ...
    int ret = vmm_map(va, (uint64_t)page, 4096, VMM_ATTR_R | VMM_ATTR_W | VMM_ATTR_NORMAL);
    if (ret != 0) {
        printk(" FAIL (map)\n");
        pmm_free_page(page);
        return -1;
...
static int test_vmm_protect(void) {
    ...
    vmm_map(va, (uint64_t)page, 4096, VMM_ATTR_R | VMM_ATTR_W | VMM_ATTR_NORMAL);
...
int memtest_run(void) {
    ...
    if (test_pmm_basic() < 0) failed++;
    if (test_pmm_patterns() < 0) failed++;
    if (test_pmm_stress() < 0) failed++;
    if (test_vmm_basic() < 0) failed++;
    if (test_vmm_protect() < 0) failed++;
    if (test_vmalloc_basic() < 0) failed++;
    if (test_vmalloc_fragmentation() < 0) failed++;
    if (test_memory_isolation() < 0) failed++;
    
    size_t free_after = pmm_free_pages_count();
    int leaked = (int)(free_before - free_after);
    
    printk("\nResults: %d/8 tests passed\n", 8 - failed);
    printk("Memory: %d pages before, %d pages after", 
