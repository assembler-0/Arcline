...
static inline uint32_t atomic_cmpxchg(volatile uint32_t *ptr, uint32_t old,
                                      uint32_t new) {
    __atomic_compare_exchange_n(ptr, &old, new, 0, __ATOMIC_SEQ_CST,
                                __ATOMIC_SEQ_CST);
    return old;
}
...
static inline uint64_t atomic_fetch_and64(volatile uint64_t *ptr,
                                          uint64_t val) {
